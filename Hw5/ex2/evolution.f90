...
subroutine flux(i, dt, FL, FR)
        ...
        !FL = 0.5 * (c*uold(i-1) + c*uold(i)) - (dx/(2.0*dt))*(uold(i)-uold(i-1))
        !FR = 0.5 * (c*uold(i) + c*uold(i+1)) - (dx/(2.0*dt))*(uold(i+1)-uold(i))

        !Lax-Wendroff method
        ul = 0.5*(uold(i)+uold(i-1)) - (dt/(2*dx))*(c*uold(i)-c*uold(i-1))
        ur = 0.5*(uold(i+1)+uold(i)) - (dt/(2*dx))*(c*uold(i+1)-c*uold(i))
        FL = c*ul
        FR = c*ur
